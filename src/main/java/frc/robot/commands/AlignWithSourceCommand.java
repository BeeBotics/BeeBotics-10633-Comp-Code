...
import frc.robot.AprilTagPID;
import frc.robot.subsystems.DriveSubsystem;
import frc.robot.subsystems.LimelightHelpers;

public class AlignWithSourceCommand extends Command {
  private DriveSubsystem m_driveSubsystem;
  private final AprilTagPID m_AprilTagPID;
  private double m_targetZ = 0.51; // fwd backward
  private double m_targetX = -0.05; // left to right
  ...
  private static final double MAX_POSITION_ERROR = 5.0; //units

  public AlignWithSourceCommand(DriveSubsystem driveSubsystem, AprilTagPID aprilTagPID) {
    m_driveSubsystem = driveSubsystem;
    m_AprilTagPID = aprilTagPID;
    // Use addRequirements() here to declare subsystem dependencies.

    NetworkTableInstance inst = NetworkTableInstance.getDefault();
    NetworkTable table = inst.getTable("SourceAlignment");
  ...
  public void initialize() {
    ...
    if (m_tidFound == false)
    {
      m_targetRotation = m_driveSubsystem.getHeading();
    }
  ...
  public void execute() {
    ...
    if (robotSpace != null && robotSpace.length >= 4) {
